...
public class ImportExport extends Activity {
	...
	public void createPDF(View v) throws Exception {
		...
		if (cb.isChecked()) {
			...
			String sub = "Relatï¿½rio diabetes - "
	...
	private static void addMetaData(Document document) {
		document.addTitle("A minha Diabetes - Exportaï¿½ï¿½o");
	...
	private void addTitlePage(Document document) throws DocumentException {
		...
		title.add(new Paragraph("Relatï¿½rio - A Minha Diabetes", titleFont));
		EditText datefrom = (EditText) findViewById(R.id.et_pdfexport_DataFrom);
		EditText dateto = (EditText) findViewById(R.id.et_pdfexport_DataTo);
	...
	private void addUserInfo(Document document) throws DocumentException,
		...
		header.add(new Paragraph("Rï¿½cio Insulina: " + obj[3].toString(),
				headerFont));
		header.add(new Paragraph("Rï¿½cio H. Carbono: " + obj[4].toString(),
				headerFont));
	...
	private void addExercise(Document document) throws DocumentException,
		...
		t.addCell(new Phrase("Duraï¿½ï¿½o (min)", tf));
		t.addCell(new Phrase("Esforï¿½o", tf));
		t.addCell(new Phrase("Notas", tf));
	...
	private void addBPressure(Document document) throws DocumentException,
		...
		addEmptyLine(title, 1);
		title.setFont(hFont);
		title.add("Leituras de Pressï¿½o Arterial");
		document.add(title);
		...
		t.addCell(new Phrase("Data", tf));
		t.addCell(new Phrase("Hora", tf));
		t.addCell(new Phrase("Sistï¿½lica", tf));
		t.addCell(new Phrase("Diastï¿½lica", tf));
		t.addCell(new Phrase("Fase do dia", tf));
		t.addCell(new Phrase("Notas", tf));
	...
	private void addDisease(Document document) throws DocumentException,
		...
		addEmptyLine(title, 1);
		title.setFont(hFont);
		title.add("Leituras de Doenï¿½as");
		document.add(title);
		...
		t.addCell(new Phrase("Doenï¿½a", tf));
		t.addCell(new Phrase("Notas", tf));
	...
	public void ShowMsgExport(String msg) {
		...
		new AlertDialog.Builder(this).setTitle("Informaï¿½ï¿½o").setMessage(msg)
				.setPositiveButton("OK", new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int whichButton) {
						// Falta verificar se nï¿½o estï¿½ associada a nenhuma
						// entrada da DB
		...
		if (isSDWriteable()) {
			...
			if (inputFile.exists()) {
				...
				try {
					...
					ShowDialogMsg("Ocurreu um erro durante o restauro, verifique se a memï¿½ria externa estï¿½ disponivel!");
				} catch (Exception exception) {
					ShowDialogMsg("Ocurreu um erro durante o restauro, verifique se a memï¿½ria externa estï¿½ disponivel!");
				}
			}
