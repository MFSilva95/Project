...
//                    throw JIPRuntimeException.create(10, clause);
    ...
    public final synchronized void addClausesDatabase(final JIPClausesDatabase db, final String strModuleName, final String strFuncName)
    {
    	db.setDynamic();

        // qui va inserita con modulo user
        m_clauseTable.put(new StringBuilder(strModuleName).append(':').append(strFuncName).toString(), db);
        //System.out.println("TESTE: "+strModuleName+" is here! ADDED: "+new StringBuilder(strModuleName).append(':').append(strFuncName).toString());
    }

    public final synchronized JIPClausesDatabase search(String funct_name){
        JIPClausesDatabase db;

        StringBuilder moduleFunName = new StringBuilder(":").append(funct_name);

        db = (JIPClausesDatabase)m_clauseTable.get(sbUSER_MODULE.resetToInitialValue().append(funct_name).toString());//":" + funct.getName());
        if(db == null)
        {
            db = (JIPClausesDatabase)m_clauseTable.get(sbSYSTEM_MODULE.resetToInitialValue().append(funct_name).toString());//":" + funct.getName());
            if(db == null)
            {
...
//            	System.out.println("\nmodule stack: " + moduleStack);
    ...
    public final synchronized JIPClausesDatabase search(final Functor funct, final Stack<String> moduleStack)
    {
    	JIPClausesDatabase db;
...
//                InputStream ins=StreamManager.getStreamManager().getInputStream("com/ugos/jiprolog/resources/jipkernel.txt","", new String[]{"jipkernel.pl"}, new String[1]);
