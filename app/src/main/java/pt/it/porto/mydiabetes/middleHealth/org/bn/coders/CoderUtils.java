...
public class CoderUtils {
    ...
    public static Field[] getOrderedFields (Class<?> objectClass){
        ...
        try {
	        for ( Field field : objectClass.getDeclaredFields() ) {
	            ASN1Element element = field.getAnnotation(ASN1Element.class);
	            if(element!=null) {
	                if(element.hasExplicitOrder())
	                    fieldOrder.put(element.declarationOrder(),field);
	                else
	                    throw new Exception ("Field ordered can not be determinated");
	            }
	        }
	        return fieldOrder.values().toArray(new Field[0]);
        }catch (Exception e){
        	//If there was not meta-information then we can not guarantee the correct order
        	e.printStackTrace();
    ...
    public static Method findGetterMethodForField(Field field, Class<?> objectClass) throws NoSuchMethodException {
        ...
        return objectClass.getMethod(getterMethodName,(Class[])null);
    }
    ...
    public static Method findIsSelectedMethodForField(Field field, Class<?> objectClass) throws NoSuchMethodException {
        ...
        return objectClass.getMethod(methodName,(Class[])null);
    }
