...
public class CarbsChartList extends MultiDataChartActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		if (savedInstanceState == null) {
			...
			extras.putParcelable(MultiDataChartActivity.EXTRAS_CHART_DATA, new pt.it.porto.mydiabetes.ui.charts.data.Carbs(this));
	...
	class ListAdapter extends GenericMultiTypeAdapter {
		...
		public void onBindViewHolder(Holder holder, int position) {
			...
			if (cursor.getString(3).equals(MyDiabetesContract.Regist.CarboHydrate.TABLE_NAME)) {
				holder.setId(0, cursor.getInt(6));
			} else if(cursor.getString(3).equals(MyDiabetesContract.Regist.BloodGlucose.TABLE_NAME)){
				holder.setId(1, cursor.getInt(6));
			}else{
				holder.setId(2, cursor.getInt(6));
			}
				...
				public void onClick(View v) {
					...
					switch (((Holder) v.getTag()).table) {
						case 0:
							...
							break;
						case 1:
							intent = new Intent(v.getContext(), GlycemiaDetail.class);
							break;
						case 2:
							intent = new Intent(v.getContext(), InsulinDetail.class);
							break;
					}
